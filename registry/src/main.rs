use actix_cors::Cors;
use actix_web::App;
use actix_web::HttpServer;
use actix_web::middleware::Compress;
use actix_web::middleware::NormalizePath;
use actix_web::middleware::TrailingSlash;
use actix_web::web;
use tracing::level_filters::LevelFilter;
use tracing_actix_web::TracingLogger;
use tracing_subscriber::EnvFilter;
use tracing_subscriber::layer::SubscriberExt as _;
use tracing_subscriber::util::SubscriberInitExt as _;

use crate::shared::blob::BlobStorage;
use crate::shared::db::Database;
use crate::util::Env;

mod features;
mod shared;
mod util;

pub struct AppState {
	database: Database,
	blob_storage: BlobStorage,
}

#[actix_web::main]
async fn main() -> std::io::Result<()> {
	let tracing_env_filter = EnvFilter::builder()
		.with_default_directive(LevelFilter::INFO.into())
		.from_env_lossy();

	let fmt_layer = tracing_subscriber::fmt::layer();

	#[cfg(debug_assertions)]
	let fmt_layer = fmt_layer.with_timer(tracing_subscriber::fmt::time::uptime());

	#[cfg(not(debug_assertions))]
	let fmt_layer = fmt_layer.with_timer(tracing_subscriber::fmt::time::time());

	tracing_subscriber::registry()
		.with(tracing_env_filter)
		.with(fmt_layer)
		.init();

	dotenvy::dotenv().unwrap();

	let address = Env::new("ADDRESS")
		.try_get()
		.await
		.unwrap_or_else(|| "0.0.0.0".to_string());
	let port = Env::new("PORT").try_parse().await.unwrap_or(8080);

	let app_state = web::Data::new(AppState {
		database: Database::new(&Env::new("DATABASE_URL").get().await).await,
		blob_storage: BlobStorage::FS(Env::new("PACKAGE_ARCHIVES_ROOT").parse().await),
	});

	HttpServer::new(move || {
		App::new()
			.app_data(app_state.clone())
			.wrap(NormalizePath::new(TrailingSlash::Trim))
			.wrap(Cors::permissive())
			.wrap(TracingLogger::default())
			.wrap(Compress::default())
			.route(
				"/",
				web::get()
					.to(async || concat!(env!("CARGO_PKG_NAME"), "/", env!("CARGO_PKG_VERSION"))),
			)
			.service(features::log::get_head::http)
			.service(features::log::get_consistency::http)
			.service(features::log::get_inclusion::http)
			.service(features::log::get_entry::http)
			.service(features::package::get_versions::http)
			.service(features::package::get_version::http)
			.service(features::package::get_archive::http)
			.service(features::package::publish::http)
			.service(features::package::yank::http)
			.service(features::package::deprecate::http)
			.service(features::scope::get_head::http)
			.service(features::scope::update_manifest::http)
			.service(features::identity::get_identity::http)
			.service(features::identity::rotate_key::http)
	})
	.bind((address, port))?
	.run()
	.await
}
